...
### 2. 公网IP获取检测
根据选择的方式进行相应检测：
- **DDNS方式**: 检测 `ip.your-domain.com` 解析
- **外部服务**: 检测 `ifconfig.me` 等服务
...
```bash
# 测试域名解析
dig element.your-domain.com @8.8.8.8
dig auth.your-domain.com @8.8.8.8
```
...
element.example.com A    123.456.789.123  # Element Web
auth.example.com    A    123.456.789.123  # 认证服务
rtc.example.com     A    123.456.789.123  # RTC服务
matrix.example.com  A    123.456.789.123  # Synapse
```
